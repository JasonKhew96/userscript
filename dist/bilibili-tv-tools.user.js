...
var backup_open = xhr_proto.open;
function new_open() {
  ...
  var method = args[0],
    ...
    user = args[3],
    password = args[4];
  if (typeof url != "string") return GMCompat.apply(this, backup_open, [method, url, async, user, password]);
  var newUrl = url;
  if (newUrl.startsWith("//")) {
    newUrl = "https:" + newUrl;
  }
  var parsedUrl = URL.parse(newUrl);
  if (!(parsedUrl != null && parsedUrl.search)) return GMCompat.apply(this, backup_open, [method, url, async, user, password]);
